...
function getOndaProdigioProductRevenueEur(paymentIntent) {
    var metadata = paymentIntent.metadata || {};
    var bumpIds = parseOrderBumps(metadata.order_bumps);
    var bumpCents = parseInt(process.env.STRIPE_BUMP_AMOUNT_CENTS || '500', 10);
    var totalCents = Number(paymentIntent.amount || 0);
    var productCents = totalCents - (bumpIds.length * bumpCents);

    if (productCents <= 0) {
        productCents = totalCents;
    }

    return Number((productCents / 100).toFixed(2));
}
...
function buildReport(paymentIntents) {
    ...
    return {
        ...
        note: 'Apenas vendas do produto Onda Prodígio (checkout9), sem upsells pós-compra, comunidade nem outros produtos. Order bumps não entram na receita. «Tráfego» = UTMs Meta ou funil pago.',
    };
}
