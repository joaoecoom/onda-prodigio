...
function isLiveCheckoutSale(paymentIntent) {
    ...
    return isLiveStripeSale(paymentIntent) &&
        funnelCheckoutConfig.isOndaProdigioFunnelCheckout(metadata.checkout);
}
...
function isOndaProdigioProductSale(paymentIntent) {
    ...
    if (!funnelCheckoutConfig.isOndaProdigioFunnelCheckout(metadata.checkout)) {
        return false;
    }
...
function isSupplementaryDashboardSale(paymentIntent) {
    if (!isLiveStripeSale(paymentIntent)) {
        return false;
    }
    ...
    if (String(metadata.checkout || '').indexOf('comprar-') === 0) {
        return true;
    }
...
function isDashboardSale(paymentIntent) {
    return isOndaProdigioProductSale(paymentIntent) || isSupplementaryDashboardSale(paymentIntent);
}
...
function normalizeSaleAttribution(sale) {
    if (sale.is_traffic) {
        return sale;
    }

    if (sale.source === 'comunidade' || sale.source === 'upsell') {
        sale.campaign_name = 'Fora do tráfego';
        sale.adset_name = sale.source_label;
        sale.ad_name = sale.product_id || sale.source_label;
        sale.ad_label = sale.product_id
            ? sale.source_label + ' · ' + sale.product_id
            : sale.source_label;
    }
...
function mapPaymentIntentToSale(paymentIntent) {
    ...
    return normalizeSaleAttribution(sale);
}
...
function buildReport(paymentIntents, options) {
    options = options || {};
    var checkoutFilter = options.checkoutFilter || null;

    var funnelSales = paymentIntents
        ...
        .map(mapPaymentIntentToSale);

    var extraSales = paymentIntents
        .filter(isSupplementaryDashboardSale)
        .map(mapPaymentIntentToSale);

    var sales = funnelSales.concat(extraSales).sort(function (a, b) {
    ...
    return {
        ...
        note: 'Receita = funil Onda Prodígio (checkout9/19, com bumps do checkout) + vendas extra na comunidade/upsells. «Tráfego» = atribuição Meta (UTM paid, fbc ou campanha). «Fora do tráfego» = orgânico, comunidade e upsells sem clique Meta.',
...
module.exports = {
    ...
    isSupplementaryDashboardSale: isSupplementaryDashboardSale,
    isDashboardSale: isDashboardSale,
    getOndaProdigioProductRevenueEur: getOndaProdigioProductRevenueEur,
    isTrafficSale: isTrafficSale,
    normalizeSaleAttribution: normalizeSaleAttribution,
