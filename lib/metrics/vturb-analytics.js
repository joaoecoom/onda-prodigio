...
var _runtimeOverride = null;

function setRuntimeOverride(config) {
    _runtimeOverride = config || null;
}

function getApiToken() {
    if (_runtimeOverride && _runtimeOverride.apiToken) {
        return String(_runtimeOverride.apiToken).trim();
    }
...
function getPlayerId() {
    ...
    if (_runtimeOverride && _runtimeOverride.playerId) {
        playerId = String(_runtimeOverride.playerId).trim();
    } else {
        playerId = (process.env.VTURB_PLAYER_ID || '').trim();
    }

    // Legacy fallback only when no offer-specific player is set.
    if (!playerId) {
        playerId = 'vid-6a7927038a043cc51fb71392';
    }
...
async function withOfferConfig(offerId, fn) {
    if (!offerId) {
        return fn();
    }

    var runtime = require('../hub/offer-runtime-config');
    var integrations = await runtime.getOfferRuntimeIntegrations(offerId);
    var previous = _runtimeOverride;
    setRuntimeOverride(runtime.vturbConfigFromIntegrations(integrations));

    try {
        return await fn();
    } finally {
        setRuntimeOverride(previous);
    }
}
...
async function buildVturbReport(query) {
    var offerId = String((query && (query.offer || query.offer_id || query.slug)) || '').trim();

    if (offerId) {
        return withOfferConfig(offerId, function () {
            return buildVturbReportInner(query);
        });
    }
...
async function buildVturbReportInner(query) {
    var token = getApiToken();
...
module.exports = {
    ...
    withOfferConfig: withOfferConfig,
    setRuntimeOverride: setRuntimeOverride,
};
