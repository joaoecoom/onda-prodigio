...
function resolveMetaDateRange(from, to, accountTimezone) {
    ...
    // Duas camadas (mantidas):
    // 1) Vendas Stripe → filtradas em Portugal (getDayStartUnix/getDayEndUnix em reporting-range.js)
    // 2) Gasto Meta → since/until = datas do filtro; USD→EUR em exchange.js
    //
    // BUG corrigido: o antigo «reporting_window» convertia meia-noite Portugal para
    // since no dia ANTERIOR na conta Meta (ex.: «Hoje» pedia 13+14 Ago → ~$51 em vez de $6).

    return {
        since: fromDate,
        until: toDate,
        adjusted: accountTz !== reportingTimezone,
