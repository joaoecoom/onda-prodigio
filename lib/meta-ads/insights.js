...
var metricsCache = require('../metrics/cache');
...
async function getAccountDetails(accountId) {
    var cacheKey = metricsCache.getCacheKey(['meta-account', accountId]);
    var accountCached = metricsCache.getCached(cacheKey, 60 * 60 * 1000);
    ...
    metricsCache.setCached(cacheKey, details);
    return details;
}
...
function getEntityIdFromRow(row, level) {
    if (level === 'campaign') {
...
function splitRangeIntoChunks(from, to, chunkDays) {
    var chunks = [];
    var cursor = from;

    while (cursor <= to) {
        var chunkEnd = metricsTimezone.addDaysIso(cursor, chunkDays - 1);
        ...
        cursor = metricsTimezone.addDaysIso(chunkEnd, 1);
    }

    return chunks;
}

async function fetchInsightsDailyRaw(accountId, from, to, level) {
    var actId = config.toActId(accountId);
...
async function getInsightsByLevelChunked(accountId, from, to, level, accountCurrency, chunkDays) {
    var currency = accountCurrency || (await getAccountDetails(accountId)).currency;
    var chunks = splitRangeIntoChunks(from, to, chunkDays);
    var grouped = {};

    for (var i = 0; i < chunks.length; i += 1) {
        var chunk = chunks[i];
        var rows = await fetchInsightsDailyRaw(accountId, chunk.from, chunk.to, level);

        rows.forEach(function (row) {
            var entityId = getEntityIdFromRow(row, level);
...
function buildInsightFilterOptions(from, to, metaDateRange) {
    return {
        reportingFrom: from,
        reportingTo: to,
        useHourlyFilter: Boolean(metaDateRange.use_hourly_filter),
        accountTimezone: metaDateRange.account_timezone,
        startUnix: metaDateRange.start_unix,
        endUnix: metaDateRange.end_unix,
        reportingTimezone: metaDateRange.reporting_timezone,
    };
}

async function getInsightsByLevelHourlyChunked(accountId, level, accountCurrency, filterOptions, chunkDays) {
    var currency = accountCurrency || (await getAccountDetails(accountId)).currency;
    var reportingFrom = filterOptions.reportingFrom;
    var reportingTo = filterOptions.reportingTo;
    var reportingTz = filterOptions.reportingTimezone || metricsTimezone.getReportingTimezone();
    var accountTz = filterOptions.accountTimezone;
    var chunks = splitRangeIntoChunks(reportingFrom, reportingTo, chunkDays);
    var grouped = {};

    for (var i = 0; i < chunks.length; i += 1) {
        var chunk = chunks[i];
        var chunkFilter = Object.assign({}, filterOptions, {
            startUnix: metricsTimezone.getDayStartUnix(chunk.from, reportingTz),
            endUnix: metricsTimezone.getDayEndUnix(chunk.to, reportingTz),
        });
        var metaFrom = metricsTimezone.formatIsoDateInTimezone(chunkFilter.startUnix, accountTz);
        var metaTo = metricsTimezone.formatIsoDateInTimezone(chunkFilter.endUnix, accountTz);
        var rows = await getInsightsByLevelHourly(accountId, metaFrom, metaTo, level, currency, chunkFilter);
    ...
    return Object.keys(grouped).map(function (entityId) {
        ...
        var sumFields = [
            ...
            'landing_page_views',
            'meta_purchases',
            'meta_purchase_value_eur',
            'meta_purchase_value_original',
            'initiate_checkout',
        ];

        for (var i = 1; i < rows.length; i += 1) {
            sumFields.forEach(function (field) {
                merged[field] = Number((Number(merged[field] || 0) + Number(rows[i][field] || 0)).toFixed(2));
            });
        }
...
async function getInsightsByLevel(accountId, from, to, level, accountCurrency, filterOptions) {
    ...
    var reportingFrom = filterOptions.reportingFrom ? filterOptions.reportingFrom : from;
    var reportingTo = filterOptions.reportingTo ? filterOptions.reportingTo : to;
    var spanDays = metricsTimezone.countDaysInRange(reportingFrom, reportingTo);

    if (filterOptions.useHourlyFilter) {
        if (spanDays > 62) {
            return getInsightsByLevelHourlyChunked(accountId, level, accountCurrency, filterOptions, 28);
        }
    ...
    if (spanDays > 62) {
        return getInsightsByLevelChunked(accountId, from, to, level, accountCurrency, 28);
    }
...
async function getAccountSummaryReport(accountId, from, to, options) {
    var skipCache = options && options.skipCache;

    if (!skipCache) {
        var cached = metaCache.getCachedSummary(accountId, from, to, metaCache.getReportTtl(from, to));
    ...
    var insightFilterOptions = buildInsightFilterOptions(from, to, metaDateRange);

    var campaignInsightRows = await getInsightsByLevel(
        accountId,
        metaFrom,
        metaTo,
        'campaign',
    ...
    var summary = summarizeNodes(campaignInsightRows, currency);
    ...
    metaCache.setCachedSummary(accountId, from, to, report);
    return report;
}
...
module.exports = {
    ...
    getAccountSummaryReport: getAccountSummaryReport,
