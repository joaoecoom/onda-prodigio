...
async function getInsightsByLevel(accountId, from, to, level) {
    var actId = config.toActId(accountId);
    ...
    return rows.map(function (row) {
        var metrics = mapInsightMetrics(row, account.currency);
...
async function getCampaignReport(accountId, from, to) {
    ...
    var adsets = await listAdsets(accountId);
    var ads = await listAds(accountId);
    var campaignInsights = indexById(await getInsightsByLevel(accountId, from, to, 'campaign'));
    var adsetInsights = indexById(await getInsightsByLevel(accountId, from, to, 'adset'));
    var adInsights = indexById(await getInsightsByLevel(accountId, from, to, 'ad'));
    ...
    adsetInsights.forEach(function (insight) {
    ...
    adInsights.forEach(function (insight) {
        var campaign = campaignRows.find(function (row) {
            return row.id === insight.campaign_id;
        });
    ...
    campaignInsights.forEach(function (insight) {
