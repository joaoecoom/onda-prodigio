...
function mapInsightMetrics(row, accountCurrency) {
    ...
    return {
        ...
        cpc_all: row.cpc ? Number(Number(row.cpc).toFixed(2)) : safeRatio(spendOriginal, clicks, 1),
        cpc_link: row.cost_per_inline_link_click
            ? Number(Number(row.cost_per_inline_link_click).toFixed(2))
            : safeRatio(spendOriginal, inlineLinkClicks, 1),
        cpm: row.cpm ? Number(Number(row.cpm).toFixed(2)) : safeRatio(spendOriginal, impressions, 1000),
        ...
        cost_per_landing_page_view: safeRatio(spendOriginal, landingPageViews, 1),
...
async function getCampaignReport(accountId, from, to, options) {
    ...
    var results = await Promise.all([
        ...
        getInsightsByLevel(accountId, metaFrom, metaTo, 'campaign', currency),
        getInsightsByLevel(accountId, metaFrom, metaTo, 'adset', currency),
        getInsightsByLevel(accountId, metaFrom, metaTo, 'ad', currency),
    ]);
