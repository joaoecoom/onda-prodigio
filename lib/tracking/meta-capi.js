...
async function sendMetaEvent(params) {
    ...
    var pixelId = tracking.meta_pixel_id || process.env.META_PIXEL_ID;
    var accessToken = tracking.meta_access_token || process.env.META_ACCESS_TOKEN;
    ...
    if (user.phone) {
        userData.ph = [identity.hashSha256(metaUserData.normalizePhoneForMetaHash(user.phone, user.phoneCountry || user.country))];
    }

    if (user.firstName) {
        var normalizedFirstName = metaUserData.normalizeMetaName(user.firstName);

        if (normalizedFirstName) {
            userData.fn = [identity.hashSha256(normalizedFirstName)];
        }
    }

    if (user.lastName) {
        var normalizedLastName = metaUserData.normalizeMetaName(user.lastName);

        if (normalizedLastName) {
            userData.ln = [identity.hashSha256(normalizedLastName)];
        }
    }

    if (user.country) {
        userData.country = metaUserData.normalizeCountryCode(user.country);
    }

    if (user.state) {
        var normalizedState = metaUserData.normalizeMetaState(user.state);

        if (normalizedState) {
            userData.st = [identity.hashSha256(normalizedState)];
        }
    }
    ...
    var eventPayload = {
        ...
        event_source_url: params.eventSourceUrl ||
            ...
            'https://onda-prodigio.vercel.app/checkout9/',
        user_data: userData,
    ...
    if (tracking.meta_test_event_code || process.env.META_TEST_EVENT_CODE) {
        payload.test_event_code = tracking.meta_test_event_code || process.env.META_TEST_EVENT_CODE;
    }
