...
function readEnvForKey(keyDef) {
    return integrationResolver.readEnvForKey(keyDef);
}
...
async function getIntegrationDetails(offerId, options) {
    ...
    integrationKeys.listIntegrationKeys().forEach(function (key) {
        ...
        var source = integrationResolver.describeKeySource(offerId, key, dbValue);
        var value = integrationResolver.resolveIntegrationValue(offerId, key, dbValue, {
        ...
        fields.push({
            ...
            source: source,
            configured: Boolean(value && value !== SECRET_MASK),
            value: value,
        });
    });
...
async function importIntegrationsFromEnv(offerId) {
    if (!integrationResolver.usesEnvIntegrationFallback(offerId)) {
        throw new Error(
            'Importação do env só está disponível para ofertas legacy (Onda Prodígio). ' +
            'Configura tracking e integrações manualmente para esta oferta.'
        );
    }
...
module.exports = {
    ...
    getStoredIntegrations: getStoredIntegrations,
