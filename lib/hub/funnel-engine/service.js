...
function createService(options) {
    ...
    async function getPageTree(offerId, pageId) {
        var page = await getPageForOffer(offerId, pageId);
        var funnel = await getFunnelForOffer(offerId, page.funnel_id);
        var sections = await repository.listSections(pageId);
        var treeSections = [];
        ...
        for (var i = 0; i < sections.length; i += 1) {
            var section = sections[i];
            var blocks = await repository.listBlocks(section.id);
            treeSections.push(Object.assign({}, section, { blocks: blocks }));
        }
    ...
    async function getPageTreeBySlugs(offerSlug, funnelSlug, pageSlug) {
        var context = await resolveOffer({ slug: offerSlug });
        ...
        return getPageTree(offerId, page.id);
    }
