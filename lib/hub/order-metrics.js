...
function summarizeFunnelBreakdown(sales) {
    var groups = {};
    var paidSales = (sales || []).filter(function (sale) {
        return sale.status !== 'refunded' && sale.is_hub_order !== false;
    });

    paidSales.forEach(function (sale) {
        var funnel = String(sale.funnel_slug || '').trim() || 'unknown';
        var page = String(sale.page_slug || '').trim() || '—';
        var key = funnel + '|' + page;

        if (!groups[key]) {
            groups[key] = {
                funnel_slug: funnel,
                funnel_label: funnel === 'unknown' ? 'Desconhecido' : funnel,
                page_slug: page === '—' ? '' : page,
                page_label: page,
                orders: 0,
                revenue_eur: 0,
        ...
        groups[key].orders += 1;
        groups[key].revenue_eur += Number(sale.amount_eur) || 0;
    });

    return Object.keys(groups).map(function (key) {
        var row = groups[key];
        row.revenue_eur = Number(row.revenue_eur.toFixed(2));
        row.aov_eur = row.orders > 0 ? Number((row.revenue_eur / row.orders).toFixed(2)) : null;
        return row;
    }).sort(function (a, b) {
...
module.exports = {
    ...
    summarizeFunnelBreakdown: summarizeFunnelBreakdown,
