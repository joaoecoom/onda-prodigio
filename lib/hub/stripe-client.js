...
async function listWebhookSecrets() {
    ...
    var e2eSecret = String(process.env.STRIPE_WEBHOOK_SECRET_E2E || '').trim();

    if (globalSecret) {
        secrets.push({ source: 'env', secret: globalSecret });
    }

    if (e2eSecret && e2eSecret !== globalSecret) {
        secrets.push({ source: 'e2e', secret: e2eSecret });
    }
...
async function verifyWebhookEvent(rawBody, signature, req) {
    ...
    var globalSecret = String(process.env.STRIPE_WEBHOOK_SECRET || '').trim();
    var testKey = String(process.env.STRIPE_TEST_SECRET_KEY || '').trim();
    var globalKey = String(process.env.STRIPE_SECRET_KEY || '').trim();
    var stripeKeys = [testKey, globalKey].filter(Boolean);

    for (var s = 0; s < stripeKeys.length; s += 1) {
        var candidates = await listWebhookSecrets();

        for (var c = 0; c < candidates.length; c += 1) {
            try {
                var stripeApi = new Stripe(stripeKeys[s]);
                var verifiedEvent = stripeApi.webhooks.constructEvent(
                    rawBody,
                    signature,
                    candidates[c].secret
                );
    ...
    var legacyCandidates = await listWebhookSecrets();

    for (var i = 0; i < legacyCandidates.length; i += 1) {
        var candidate = legacyCandidates[i];
