...
async function updateOfferSettings(slug, patch) {
    ...
    var settings = Object.assign({}, offer.settings || {});
    ...
    if (patch.commercial_currency != null) {
        settings.commercial_currency = integrationResolver.normalizeCurrency(patch.commercial_currency);
        updates.settings = settings;
    }

    if (patch.settings && typeof patch.settings === 'object') {
        Object.keys(patch.settings).forEach(function (key) {
            if (patch.settings[key] != null) {
                settings[key] = patch.settings[key];
            }
        });
        updates.settings = settings;
    }
    ...
    if (patch.commercial_currency != null) {
        ...
        try {
            await offerProvisioning.updateMainCheckout(offer.id, {
                currency: settings.commercial_currency,
            });
        } catch (checkoutError) {
            // Checkout may not exist yet during early setup.
        }
    }
