...
function checkDomain(offer, domainRows, domainStatus) {
    ...
    if (!vercelDomains.isVercelConfigured()) {
        return makeCheck({
            ...
            status: 'warning',
            severity: 'important',
            message: domain + ' — validação Vercel pendente.',
            cause: 'VERCEL_TOKEN/VERCEL_PROJECT_ID não configurados no runtime.',
            solution: 'Configura credenciais Vercel ou valida DNS manualmente.',
