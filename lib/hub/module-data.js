...
async function getCheckoutModule(offer) {
    ...
    var checkouts = await offerProvisioning.listOfferCheckouts(offer.id);
    var checkout = checkouts.find(function (row) {
        return row.checkout_id === 'main';
    }) || checkouts[0] || (offer.checkouts || [])[0] || {};
    ...
    return {
        ...
        checkout: {
            ...
            label: checkout.label || 'Checkout Principal',
            is_active: checkout.is_active !== false,
        },
        checkouts: checkouts.map(function (row) {
            return {
