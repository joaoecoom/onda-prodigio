...
async function getRenderablePageBySlugs(offerSlug, funnelSlug, pageSlug, options) {
    ...
    var tree = validateRenderableTree(
        await funnelEngine.getPageTreeBySlugs(offerSlug, funnelSlug, pageSlug, { offer: context })
    );
...
async function renderPageHtml(input, options) {
    ...
    if (payload.tree.funnel && payload.tree.funnel.type === 'quiz') {
        var quizEngine = require('../quiz-engine');
        var quiz = await quizEngine.loadQuizBundle(payload.tree.funnel.id, payload.offer.id);
        html = quizEngine.renderQuizDocument({
            offer: payload.offer,
            funnel: payload.tree.funnel,
            page: payload.tree.page,
            quiz: quiz,
        }, {
        ...
        html = pageRenderer.renderPageDocument(payload.tree, {
            mode: opts.mode || (payload.tree.page.status === 'published' ? 'production' : 'preview'),
            showPreviewBanner: Boolean(opts.showPreviewBanner || payload.tree.page.status !== 'published'),
