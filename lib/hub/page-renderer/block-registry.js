...
function renderHeading(block, ctx) {
    ...
    var tag = 'h' + level;
    var text = escapeUtil.escapeHtml(content.text || '');
    var align = settings.alignment || content.alignment || 'left';
    var inner = '<' + tag + ' class="pe-heading" style="text-align:' +
...
function renderHtml(block, ctx) {
    ...
    var warning = mode === 'preview'
        ? '<p class="pe-html-warning">HTML block — conteúdo raw (scripts removidos)</p>'
        : '';
    var inner = warning + '<div class="pe-html-raw">' + sanitized + '</div>';
