...
var MODE_PROMPTS = {
    ...
    checkout: [
        ...
        'Layout padrão: vertical dark — imagem/produto em cima, dados pessoais, order bumps (Comprar junto), detalhes da compra, pagamento Stripe.',
        'Usa save_checkout_template só para chrome (scarcity, testemunhos, CSS). NÃO replaces o formulário core.',
        'NÃO removes nem renomeias IDs obrigatórios: checkout-form, payment-element, submit-payment, order-bump-list, order-bumps-section, checkout-product-name, checkout-price.',
        'Preço principal: update_checkout_pricing (amount_cents, currency, label).',
        'Order bumps: upsert_order_bump (bump_id, product_id, label, amount_cents). Cria product_id Hub se preciso.',
        'OBRIGATÓRIO após qualquer mudança de preço/produto/bump: o HUB sincroniza Stripe automaticamente (produto+price+webhook).',
        'Se stripe_sync.skipped=stripe_not_configured → pede secret key e usa save_offer_integrations, depois configure_stripe_catalog + ensure_stripe_webhook.',
        'Nunca deixes preços só no HTML — preço vem sempre do HUB/Stripe.',
        'Responde em português de Portugal.',
    ].join('\n'),
    page: [
