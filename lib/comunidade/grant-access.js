...
function parseLegacyOrderBumps(metadata) {
...
function parseProductIdsFromMetadata(metadata) {
    ...
    if (productId && isUniversalOfferCheckout(metadata)) {
        var orderBumpProducts = require('../hub/order-bumps').parseProductIdsFromMetadata(metadata);

        if (orderBumpProducts.length) {
    ...
    return parseLegacyOrderBumps(metadata);
}

function isUniversalOfferCheckout(metadata) {
    var data = metadata || {};

    return data.checkout_type === 'offer' || data.checkout === 'main';
}

function shouldGrantAccessForPayment(metadata, paymentIntent) {
    if (!paymentIntent || paymentIntent.status !== 'succeeded') {
        return false;
    }

    if (isUniversalOfferCheckout(metadata)) {
        return parseProductIdsFromMetadata(metadata).length > 0;
    }

    return isLiveStripeMetadata(metadata);
}
...
async function grantAccessFromPaymentIntent(paymentIntent) {
    ...
    if (!shouldGrantAccessForPayment(metadata, paymentIntent)) {
    ...
    var productIds = parseProductIdsFromMetadata(metadata);
    ...
    var purchasedAt = paymentIntent.created
        ? new Date(Number(paymentIntent.created) * 1000).toISOString()
        : new Date().toISOString();
    var followUpQueueResult = await neverLoggedInQueue.enqueueNeverLoggedInFollowUp({
        admin: admin,
        memberId: member.id,
    ...
    return {
        ...
        never_logged_in_queue: followUpQueueResult,
    };
}
...
async function grantAccessFromCheckoutSession(stripe, session) {
    ...
    var purchasedAt = session.created
...
module.exports = {
    ...
    parseLegacyOrderBumps: parseLegacyOrderBumps,
    parseProductIdsFromMetadata: parseProductIdsFromMetadata,
    isUniversalOfferCheckout: isUniversalOfferCheckout,
    shouldGrantAccessForPayment: shouldGrantAccessForPayment,
    parseOrderBumps: parseProductIdsFromMetadata,
