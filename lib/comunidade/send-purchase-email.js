 ...
 * }} options
 */
async function resolveOfferIdFromProducts(admin, productIds) {
    if (!admin || !productIds || !productIds.length) {
        return '';
    }
    ...
    try {
        ...
        return String(result.data[0].offer_id || '').trim();
    } catch (error) {
        return '';
    }
}
...
async function maybeSendAfterGrant(options) {
    ...
    var offerId = options.offerId || options.offer_id ||
        await resolveOfferIdFromProducts(admin, options.productIds);
