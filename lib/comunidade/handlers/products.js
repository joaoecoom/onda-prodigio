...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        var accessibleProductIds = await authHelpers.getAccessibleProductIds(
            auth.admin,
            member,
            adminProfile,
            { offerId: offerId }
        ...
        var productsQuery = auth.admin
            .from('products')
            .select('id, name, description, image_url, sort_order, offer_id')
            .order('sort_order', { ascending: true });

        if (offerId) {
            productsQuery = productsQuery.eq('offer_id', offerId);
        }

        var productsResult = await productsQuery;
