...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'DELETE') {
            return handleDelete(req, res, auth, adminProfile);
        }
...
async function handleDelete(req, res, auth, adminProfile) {
    if (!adminProfile) {
        return authHelpers.jsonError(res, 403, 'Apenas administradores podem eliminar comentários.');
    }
    ...
    var productIds = await authHelpers.getAccessibleProductIds(auth.admin, null, adminProfile);
