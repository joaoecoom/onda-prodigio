...
async function processNextNeverLoggedInWhatsApp() {
    ...
    if (!phoneDigits) {
        await markQueueItem(admin, member.id, 'cancelled', 'missing_phone');
        return {
            ok: true,
            done: false,
            skipped: true,
    ...
    if (sendResult.ok) {
        ...
        return {
            ...
            sent: {
                email: member.email,
                phone: sendResult.phone || phoneDigits,
    ...
    if (sendResult.skipped) {
        await markQueueItem(admin, member.id, 'cancelled', sendResult.reason || 'skipped');
