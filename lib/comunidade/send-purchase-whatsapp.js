...
async function maybeSendAfterGrant(options) {
    ...
    var offerId = options.offerId || options.offer_id || '';

    if (!offerId && options.productIds && options.productIds.length) {
        try {
            ...
            if (!productResult.error && productResult.data && productResult.data[0]) {
                offerId = String(productResult.data[0].offer_id || '').trim();
            }
        } catch (error) {
            offerId = '';
        }
    ...
    // Env-only enable check kept for global kill-switch; offer config can still enable send.
    if (!evolutionApi.isEnabled() && !offerId) {
