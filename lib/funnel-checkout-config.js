var UNIVERSAL_CHECKOUT_ID = 'main';
...
function resolveCheckoutId(raw) {
    ...
    if (id === UNIVERSAL_CHECKOUT_ID) {
        return UNIVERSAL_CHECKOUT_ID;
    }
...
function getCheckoutConfig(checkoutId, mode) {
    ...
    if (isTest) {
        return {
            checkoutId: config.testId || config.id,
            amountCents: config.defaultAmountCents,
            priceId: '',
            checkoutPath: config.testPath || config.path,
            thankYouPath: config.id === UNIVERSAL_CHECKOUT_ID ? '/comunidade/' : '/obgd-test/',
            label: config.label,
    ...
    return {
        ...
        amountCents: config.amountCentsEnv
            ? parseInt(process.env[config.amountCentsEnv] || String(config.defaultAmountCents), 10)
            : config.defaultAmountCents,
        priceId: config.priceIdEnv ? (process.env[config.priceIdEnv] || '') : '',
        checkoutPath: config.path,
        thankYouPath: config.id === UNIVERSAL_CHECKOUT_ID ? '/comunidade/' : '/obgd/',
        label: config.label,
        sourceCheckoutId: config.id,
    };
}
...
module.exports = {
    UNIVERSAL_CHECKOUT_ID: UNIVERSAL_CHECKOUT_ID,
